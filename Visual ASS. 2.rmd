...
basemap %>%
  addMinicharts(
    data1$LON, data1$LAT,
    type = "pie",
    chartdata = data1[, c("DEATH", "RECOVERED", "CONFIRMED")], 
    colorPalette = colors, 
    width = 60 * sqrt(data1$POP) / sqrt(max(data1$POP)), transitionTime = 0
  )
```
...
H <-leaflet() %>% 
  addTiles() %>% 
  setView( lat=-19, lng=30 , zoom=4) %>% 
  addHeatmap(group = "LITERACY_PER_CENT", 
    data = data1, 
    lng = ~ LON,
    lat = ~ LAT, 
    intensity = ~LITERACY_PER_CENT, 
    radius = 8
  ) %>% 
  addHeatmap(group = "DEATHRATE", 
    data = data1, 
    lng = ~ LON,
    lat = ~ LAT, 
    intensity = ~DEATHRATE, 
  ...
  addHeatmap(group = "POP", 
   ...
   addLayersControl(
    baseGroups = c("POP", "DEATHRATE", "LITERACY_PER_CENT"), 
    position = "bottomleft", 
...
Row
---------------------------------------
### COVID DEATHS vs LITERACY PER CENT

```{r}
b  = ggplot(data1, aes(x = DEATH, y = LITERACY_PER_CENT  )) + 
  stat_binhex(bins = 10)
b
```
...
Row
---------------------------------------
### COVID DEATHS vs INHERENT  DEATH RATE 
```{r}
#Display values
C  = ggplot(data1, aes(x = DEATH, y =  DEATHRATE   )) + 
  stat_binhex(bins = 10)
C
```
...
```{r}
#Display values
C  = ggplot(data1, aes(x = DEATH, y =  GDP   )) + 
  stat_binhex(bins = 10)
C
```
